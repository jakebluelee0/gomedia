...
import (
	...
	"github.com/jakebluelee0/gomedia/core/media/av/pktque"
)
...
func NewTranscoder(streams []av.CodecData, options Options) (_self *Transcoder, err error) {
	...
	for i, stream := range streams {
		...
		demuxer.streams = append(demuxer.streams, ts)
	}
...
func (stream *tStream) audioDecodeAndEncode(inpkt av.Packet) (outpkts []av.Packet, err error) {
	var dur time.Duration
	var frame av.AudioFrame
	var ok bool
	if ok, frame, err = stream.adec.Decode(inpkt.Data); err != nil {
		return
	}
	if !ok {
		return
	...
	if dur, err = stream.adecodec.PacketDuration(inpkt.Data); err != nil {
	...
	stream.timeline.Push(inpkt.Time, dur)

	var _outpkts [][]byte
	if _outpkts, err = stream.aenc.Encode(frame); err != nil {
		return
	}
	for _, _outpkt := range _outpkts {
		if dur, err = stream.aencodec.PacketDuration(_outpkt); err != nil {
			err = fmt.Errorf("transcode: PacketDuration() failed for output stream #%d", inpkt.Idx)
			return
		}
		outpkt := av.Packet{Idx: inpkt.Idx, Data: _outpkt}
		outpkt.Time = stream.timeline.Pop(dur)
...
func (trans *Transcoder) Do(pkt av.Packet) (out []av.Packet, err error) {
	stream := trans.streams[pkt.Idx]
...
func (trans *Transcoder) Streams() ([]av.CodecData, error) {
...
func (muxer *Muxer) WriteHeader(streams []av.CodecData) (err error) {
	if muxer.transcoder, err = NewTranscoder(streams, muxer.Options); err != nil {
		return
	}
	var newstreams []av.CodecData
	if newstreams, err = muxer.transcoder.Streams(); err != nil {
		return
	}
	if err = muxer.Muxer.WriteHeader(newstreams); err != nil {
		return
	}
	return
}
...
func (muxer *Muxer) WritePacket(pkt av.Packet) (err error) {
	...
	if outpkts, err = muxer.transcoder.Do(pkt); err != nil {
		return
	}
	for _, pkt := range outpkts {
		if err = muxer.Muxer.WritePacket(pkt); err != nil {
			return
		}
	}
	return
...
// Close func
func (muxer *Muxer) Close() (err error) {
	if muxer.transcoder != nil {
		return muxer.transcoder.Close()
	}
	return
}
...
func (demuxer *Demuxer) prepare() (err error) {
	if demuxer.transcoder == nil {
		var streams []av.CodecData
		if streams, err = demuxer.Demuxer.Streams(); err != nil {
			return
		}
		if demuxer.transcoder, err = NewTranscoder(streams, demuxer.Options); err != nil {
			return
		}
	}
	return
...
func (demuxer *Demuxer) ReadPacket() (pkt av.Packet, err error) {
	...
	for {
		if len(demuxer.outpkts) > 0 {
			pkt = demuxer.outpkts[0]
			demuxer.outpkts = demuxer.outpkts[1:]
			return
		}
		var rpkt av.Packet
		if rpkt, err = demuxer.Demuxer.ReadPacket(); err != nil {
			return
		}
		if demuxer.outpkts, err = demuxer.transcoder.Do(rpkt); err != nil {
			return
		}
	}
	// return
...
func (demuxer *Demuxer) Streams() (streams []av.CodecData, err error) {
	...
	return demuxer.transcoder.Streams()
}

// Close func
func (demuxer *Demuxer) Close() (err error) {
	if demuxer.transcoder != nil {
		return demuxer.transcoder.Close()
	}
	return
}
