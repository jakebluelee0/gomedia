...
func ParseSPS(data []byte) (spsInfo SPSInfo, err error) {
	...
	if spsInfo.ProfileIdc, err = r.ReadBits(8); err != nil {
		return
	}
	...
	// level_idc
	if spsInfo.LevelIdc, err = r.ReadBits(8); err != nil {
		return
	}
	...
	if spsInfo.ProfileIdc == 100 || spsInfo.ProfileIdc == 110 ||
		spsInfo.ProfileIdc == 122 || spsInfo.ProfileIdc == 244 ||
		spsInfo.ProfileIdc == 44 || spsInfo.ProfileIdc == 83 ||
		spsInfo.ProfileIdc == 86 || spsInfo.ProfileIdc == 118 {
	...
	if spsInfo.MbWidth, err = r.ReadExponentialGolombCode(); err != nil {
		return
	}
	spsInfo.MbWidth++

	if spsInfo.MbHeight, err = r.ReadExponentialGolombCode(); err != nil {
		return
	}
	spsInfo.MbHeight++
	...
	if frameCroppingFlag != 0 {
		if spsInfo.CropLeft, err = r.ReadExponentialGolombCode(); err != nil {
			return
		}
		if spsInfo.CropRight, err = r.ReadExponentialGolombCode(); err != nil {
			return
		}
		if spsInfo.CropTop, err = r.ReadExponentialGolombCode(); err != nil {
			return
		}
		if spsInfo.CropBottom, err = r.ReadExponentialGolombCode(); err != nil {
			return
		}
	}

	spsInfo.Width = (spsInfo.MbWidth * 16) - spsInfo.CropLeft*2 - spsInfo.CropRight*2
	spsInfo.Height = ((2 - frameMbsOnlyFlag) * spsInfo.MbHeight * 16) - spsInfo.CropTop*2 - spsInfo.CropBottom*2
...
func (instance CodecData) SPS() []byte {
	return instance.RecordInfo.SPS[0]
}

// PPS func
func (instance CodecData) PPS() []byte {
	return instance.RecordInfo.PPS[0]
}

// Width func
func (instance CodecData) Width() int {
	return int(instance.SPSInfo.Width)
}

// Height func
func (instance CodecData) Height() int {
	return int(instance.SPSInfo.Height)
}
...
func NewCodecDataFromAVCDecoderConfRecord(record []byte) (instance CodecData, err error) {
	...
	if _, err = (&instance.RecordInfo).Unmarshal(record); err != nil {
		return
	}
	if len(instance.RecordInfo.SPS) == 0 {
		err = fmt.Errorf("h264parser: no SPS found in AVCDecoderConfRecord")
		return
	}
	if len(instance.RecordInfo.PPS) == 0 {
		err = fmt.Errorf("h264parser: no PPS found in AVCDecoderConfRecord")
		return
	}
	if instance.SPSInfo, err = ParseSPS(instance.RecordInfo.SPS[0]); err != nil {
...
func NewCodecDataFromSPSAndPPS(sps, pps []byte) (instance CodecData, err error) {
	...
	instance.RecordInfo = recordinfo
	instance.Record = buf

	if instance.SPSInfo, err = ParseSPS(sps); err != nil {
		return
	}
	return
}
...
func (instance *AVCDecoderConfRecord) Unmarshal(b []byte) (n int, err error) {
	...
	instance.AVCProfileIndication = b[1]
	instance.ProfileCompatibility = b[2]
	instance.AVCLevelIndication = b[3]
	instance.LengthSizeMinusOne = b[4] & 0x03
	...
	for i := 0; i < spscount; i++ {
		...
		instance.SPS = append(instance.SPS, b[n:n+spslen])
		n += spslen
	}
	...
	for i := 0; i < ppscount; i++ {
		...
		instance.PPS = append(instance.PPS, b[n:n+ppslen])
		n += ppslen
	}
...
func (instance AVCDecoderConfRecord) Len() (n int) {
	n = 7
	for _, sps := range instance.SPS {
		n += 2 + len(sps)
	}
	for _, pps := range instance.PPS {
		n += 2 + len(pps)
	}
	return
}
...
func (instance AVCDecoderConfRecord) Marshal(b []byte) (n int) {
	b[0] = 1
	b[1] = instance.AVCProfileIndication
	b[2] = instance.ProfileCompatibility
	b[3] = instance.AVCLevelIndication
	b[4] = instance.LengthSizeMinusOne | 0xfc
	b[5] = uint8(len(instance.SPS)) | 0xe0
	n += 6
