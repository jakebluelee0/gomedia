...
import (
	...
	"github.com/jakebluelee0/gomedia/core/media/utils/bits"
...
Instantaneous Decoder Refresh (IDR). This VCL NALU is a inst contained image slice. That is, an IDR can be decoded and displayed without referencing any other NALU save SPS and PPS.
...
func ParseSPS(data []byte) (inst SPSInfo, err error) {
	...
	if inst.ProfileIdc, err = r.ReadBits(8); err != nil {
		return
	}
	...
	// level_idc
	if inst.LevelIdc, err = r.ReadBits(8); err != nil {
		return
	}
	...
	if inst.ProfileIdc == 100 || inst.ProfileIdc == 110 ||
		inst.ProfileIdc == 122 || inst.ProfileIdc == 244 ||
		inst.ProfileIdc == 44 || inst.ProfileIdc == 83 ||
		inst.ProfileIdc == 86 || inst.ProfileIdc == 118 {
	...
	if inst.MbWidth, err = r.ReadExponentialGolombCode(); err != nil {
		return
	}
	inst.MbWidth++

	if inst.MbHeight, err = r.ReadExponentialGolombCode(); err != nil {
		return
	}
	inst.MbHeight++
	...
	if frameCroppingFlag != 0 {
		if inst.CropLeft, err = r.ReadExponentialGolombCode(); err != nil {
			return
		}
		if inst.CropRight, err = r.ReadExponentialGolombCode(); err != nil {
			return
		}
		if inst.CropTop, err = r.ReadExponentialGolombCode(); err != nil {
			return
		}
		if inst.CropBottom, err = r.ReadExponentialGolombCode(); err != nil {
			return
		}
	}

	inst.Width = (inst.MbWidth * 16) - inst.CropLeft*2 - inst.CropRight*2
	inst.Height = ((2 - frameMbsOnlyFlag) * inst.MbHeight * 16) - inst.CropTop*2 - inst.CropBottom*2
...
func (codecData CodecData) AVCDecoderConfRecordBytes() []byte {
	return codecData.Record
}

// SPS func
func (codecData CodecData) SPS() []byte {
	return codecData.RecordInfo.SPS[0]
}

// PPS func
func (codecData CodecData) PPS() []byte {
	return codecData.RecordInfo.PPS[0]
}

// Width func
func (codecData CodecData) Width() int {
	return int(codecData.SPSInfo.Width)
}

// Height func
func (codecData CodecData) Height() int {
	return int(codecData.SPSInfo.Height)
}
...
func NewCodecDataFromSPSAndPPS(sps, pps []byte) (codecData CodecData, err error) {
	...
	codecData.RecordInfo = recordinfo
	codecData.Record = buf

	if codecData.SPSInfo, err = ParseSPS(sps); err != nil {
		return
	}
	return
}
...
func (confRecord *AVCDecoderConfRecord) Unmarshal(b []byte) (n int, err error) {
	...
	confRecord.AVCProfileIndication = b[1]
	confRecord.ProfileCompatibility = b[2]
	confRecord.AVCLevelIndication = b[3]
	confRecord.LengthSizeMinusOne = b[4] & 0x03
	...
	for i := 0; i < spscount; i++ {
		...
		confRecord.SPS = append(confRecord.SPS, b[n:n+spslen])
		n += spslen
	}
	...
	for i := 0; i < ppscount; i++ {
		...
		confRecord.PPS = append(confRecord.PPS, b[n:n+ppslen])
		n += ppslen
	}
...
func (confRecord AVCDecoderConfRecord) Len() (n int) {
	n = 7
	for _, sps := range confRecord.SPS {
		n += 2 + len(sps)
	}
	for _, pps := range confRecord.PPS {
		n += 2 + len(pps)
	}
	return
}
...
func (confRecord AVCDecoderConfRecord) Marshal(b []byte) (n int) {
	b[0] = 1
	b[1] = confRecord.AVCProfileIndication
	b[2] = confRecord.ProfileCompatibility
	b[3] = confRecord.AVCLevelIndication
	b[4] = confRecord.LengthSizeMinusOne | 0xfc
	b[5] = uint8(len(confRecord.SPS)) | 0xe0
	n += 6
...
func ParseSliceHeaderFromNALU(packet []byte) (sliceType SliceType, err error) {
	...
	default:
		err = fmt.Errorf("h264parser: nalUnitType=%d has no slice header", nalUnitType)
		return
	}
